...
const focusTrap = (node, firstFocusableElementIndex = 0) => {
  const focusableContent = node.querySelectorAll(focusableElements);
  const firstFocusableElement = focusableContent[firstFocusableElementIndex];
...
if (burger) {
  ...
  const POS_Y = 4;
  const ROTATE_DEG = 45;
  const burgerTopLineAnimation = gsapWithCSS.fromTo(
    burgerTopLine,
    {
      y: 0,
      rotate: 0
  ...
  const burgerBottomLineAnimation = gsapWithCSS.fromTo(
    burgerBottomLine,
    {
      y: 0,
      rotate: 0
  ...
  const navMenuWrapperAnimation = gsapWithCSS.fromTo(
    navMenuWrapper,
    {
      x: "100vw"
    },
...
const GOOGLE_CAPTHCA_V_3_SITE_KEY = "6LcqKtsrAAAAADP-s2exb2byUIAwPQ9s-QOD3okN";
function sendForm(form) {
  ...
  grecaptcha.ready(function() {
    grecaptcha.execute(GOOGLE_CAPTHCA_V_3_SITE_KEY, { action: "submit" }).then(function(token) {
      form.querySelector(".g-recaptcha-response").value = token;
      console.log("TOKEN", token);
      const data = new FormData(form);
      ajax(form.method, form.action, data, success, error);
      function ajax(method, url, data2, success2, error2) {
        ...
        xhr.onreadystatechange = function() {
          ...
          if (xhr.status === 200) {
            success2(xhr.response, xhr.responseType);
          } else {
            error2(xhr.status, xhr.response, xhr.responseType);
          }
        };
        xhr.send(data2);
      }
